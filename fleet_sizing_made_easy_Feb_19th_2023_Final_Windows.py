...
def distribution2(LOWER_LIMIT, MU, SIGMA):
    ...
    while flag:
        ...
        if random_number < LOWER_LIMIT:
            pass
        elif random_number >= LOWER_LIMIT:
            flag = False
...
def main(STARTING_INVENTORY_UNITS, BATCH_PRODUCTION_AMOUNT, PRODUCTION_INVERVAL, INVENTORY_DRAW_THRESHOLD, FLEET_SIZE, VEHICLE_CAPACITY, PICKUP_PERIOD, LOWER_LIMIT=0, MU=0, SIGMA=0, EVALUATION_INTERVAL=0, PRODUCTION_PERIODS_ON=1, PRODUCTION_PERIODS_OFF=0):
    ...
    '''
    1)  STARTING_INVENTORY_UNITS
    2)  BATCH_PRODUCTION_AMOUNT
    3)  PRODUCTION_INVERVAL
    4)  INVENTORY_DRAW_THRESHOLD
    5)  FLEET_SIZE
    6)  VEHICLE_CAPACITY
    7)  PICKUP_PERIOD
    8)  LOWER_LIMIT
    9)  MU
    10) SIGMA
    11) EVALUATION_INTERVAL
    12) PRODUCTION_PERIODS_ON
    13) PRODUCTION_PERIODS_OFF
        ...
        while period_counter < EVALUATION_INTERVAL:
            ...
            if eop_inventory[-1] >= INVENTORY_DRAW_THRESHOLD+VEHICLE_CAPACITY and available_vehicles > 0 and pickup_counter == PICKUP_PERIOD:
                ...
                for load in range(1,loads+1):
                    ...
                    turnaround_times_for_vehicles.append(distribution2(LOWER_LIMIT, MU, SIGMA))
            ...
            for index, (vehicle,time) in enumerate(zip(in_transit_vehicles, turnaround_times_for_vehicles)): 
             

                if vehicle > time:
                    in_transit_vehicles.pop(index)
                    turnaround_times_for_vehicles.pop(index)
                    available_vehicles += 1

                    vehicle_turnaround_record.append(vehicle-1)
...
second_frame.place(relx = 0.5, y=10, relwidth=.4, height=100, anchor='n')
...
label = tk.Label(second_frame, text='Copyright belongs to Sebastian Puchalski, 2023',bg=CANVAS_COLOR,fg=FRAME_COLOR, font='Helvetica 10 bold')
...
label = tk.Label(second_frame, text='Turnaround time, avg.[periods]',bg=LABEL_COLOR,fg= FG_LABEL_COLOR,font=FONT)
label.place(relx=0, relwidth=0.7, relheight=.9)
